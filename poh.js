...
(function(){
  ...
  const defaultFossilWeights = {
    'Metallic Fossil': 0.0990,
    'Frigid Fossil': 0.1086,
    'Serrated Fossil': 0.0288,
    'Fundamental Fossil': 0.016,
    'Prismatic Fossil': 0.0288,
    'Pristine Fossil': 0.0895,
    'Dense Fossil': 0.115,
    'Aberrant Fossil': 0.1054,
    'Aetheric Fossil': 0.0288,
    'Bound Fossil': 0.0288,
    'Corroded Fossil': 0.0256,
    'Bloodstained Fossil': 0.003,
    'Hollow Fossil': 0.0032,
    'Faceted Fossil': 0.003,
    'Jagged Fossil': 0.1086,
    'Gilded Fossil': 0.0064,
    'Opulent Fossil': 0.0224,
    'Lucent Fossil': 0.0224,
    'Fractured Fossil': 0.003,
    'Tangled Fossil': 0.003,
    'Deft Fossil': 0.0319,
    'Scorched Fossil': 0.1086
  };
  ...
  function computeFossilStrategy(){ const data = readFossilData(); const types = Object.keys(data.prices); const pricesArr = types.map(t=> data.prices[t]); const weightsArr = types.map(t=> data.weights[t]); const purplePerDiv = Number(document.getElementById('f_purplePerDiv')?.value) || 0; const chaosPerDiv = Number(document.getElementById('f_chaosPerDiv')?.value) || 0; let cost=0; if(chaosPerDiv>0 && purplePerDiv>0){ cost = 30 * (chaosPerDiv/purplePerDiv); } else if(fetchedChaosPerLifeforce!=null){ cost = 30 * fetchedChaosPerLifeforce; } else cost = 0; const C = computeFossilThreshold(pricesArr, weightsArr, cost); const V={}; types.forEach((t,i)=> V[t] = Math.max(data.prices[t], C)); const start=document.getElementById('f_startType')?.value; const count = Math.max(1, Math.floor(Number(document.getElementById('f_startCount')?.value)||1)); const expectedPer = V[start]; const totalExpected = expectedPer * count; const immediate = data.prices[start] * count; // gross improvement (ignores craft cost)
    const grossGain = totalExpected - immediate;
    const stopTypes = types.filter(t=> data.prices[t] >= C);
    // q = probability a single random swap yields a stop-type (weighted)
    const q = stopTypes.reduce((s,t)=> s + (data.weights[t]||0), 0);
    let expectedFlipsPerFossil;
    if(data.prices[start] >= C) {
      expectedFlipsPerFossil = 0;
    } else if(q > 0) {
      expectedFlipsPerFossil = 1 / q;
    } else {
      // If no stop types exist (q==0) and start < C, flipping will never reach a stop; mark as Infinity
      expectedFlipsPerFossil = Number.POSITIVE_INFINITY;
    }
    const totalExpectedCraftCost = (expectedFlipsPerFossil === Number.POSITIVE_INFINITY) ? Number.POSITIVE_INFINITY : (count * expectedFlipsPerFossil * cost);
    const netProfit = (totalExpectedCraftCost === Number.POSITIVE_INFINITY) ? Number.NEGATIVE_INFINITY : (grossGain - totalExpectedCraftCost);
    const res = document.getElementById('fossilResults'); if(!res) return; res.innerHTML=''; function fmtChaosDiv(v){ if(!isFinite(v)) return (v === Number.POSITIVE_INFINITY) ? '∞' : (v === Number.NEGATIVE_INFINITY) ? '-∞' : '—'; if(chaosPerDiv>0) return `${v.toFixed(3)} chaos / ${(v/chaosPerDiv).toFixed(3)} div`; return `${v.toFixed(3)} chaos / — div`; } const summary=document.createElement('div'); summary.innerHTML = `<strong>Threshold (C):</strong> ${fmtChaosDiv(C)} — <strong>Stop types:</strong> ${stopTypes.join(', ')}`; res.appendChild(summary); const table = document.createElement('table'); table.className='results-table'; table.style.marginTop='10px'; const cg=document.createElement('colgroup'); for(let i=0;i<4;i++) cg.appendChild(document.createElement('col')); table.appendChild(cg); const hdr=document.createElement('tr'); hdr.innerHTML = '<th style="text-align:left">Type</th><th style="text-align:left">Price (c)</th><th style="text-align:left">Optimal Value (c)</th><th style="text-align:left">Action</th>'; table.appendChild(hdr); const sorted = types.slice().sort((a,b)=> (data.prices[b]||0) - (data.prices[a]||0)); sorted.forEach(t=>{ const tr=document.createElement('tr'); tr.innerHTML = `<td>${t}</td><td>${data.prices[t].toFixed(2)}</td><td>${V[t].toFixed(3)}</td><td>${data.prices[t]>=C? 'Keep / Stop' : 'Flip'}</td>`; table.appendChild(tr); }); res.appendChild(table); const footer=document.createElement('div'); footer.style.marginTop='10px'; footer.innerHTML = `<div><strong>Start:</strong> ${count} × ${start} (price ${fmtChaosDiv(data.prices[start])})</div><div><strong>Craft per flip (computed):</strong> ${fmtChaosDiv(cost)}</div><div><strong>Expected final per fossil:</strong> ${fmtChaosDiv(expectedPer)}</div><div><strong>Expected flips per fossil:</strong> ${isFinite(expectedFlipsPerFossil)? expectedFlipsPerFossil.toFixed(3) : '∞'}</div><div><strong>Expected craft cost:</strong> ${fmtChaosDiv(totalExpectedCraftCost)}</div><div><strong>Expected total value:</strong> ${fmtChaosDiv(totalExpected)}</div><div style="color:${netProfit>=0? 'var(--accent)': '#f87171'}"><strong>Expected net profit (after craft):</strong> ${fmtChaosDiv(netProfit)}</div><div style="color:${grossGain>=0? 'var(--accent)': '#f87171'}"><strong>Gross (no craft cost):</strong> ${fmtChaosDiv(grossGain)}</div>`; res.appendChild(footer);
  }
